...
type ColumnType = ColumnDef<StrategyInterface, unknown>;
...
export function StrategyListPage(props: StrategyListPageProps) {
  ...
  const fetcher = useCallback(
    async () =>
      getStrategies({
        relations: ['user', 'feedback.count', 'order.count'],
  ...
  const { data, error, isLoading, mutate } = useSWR<PaginatedInterface<StrategyInterface>>(
    () => `/strategies?params=${JSON.stringify(params)}`,
    fetcher,
  );
  ...
  const handleView = (row: StrategyInterface) => {
    if (hasAccess('strategy', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)) {
      router.push(`/strategies/view/${row.id}`);
    }
  };
...

    {
      ...
      cell: ({ row: { original: record } }: any) => (
        <Flex justifyContent="flex-end">
          <NextLink href={`/strategies/view/${record.id}`} passHref legacyBehavior>
            <Button
